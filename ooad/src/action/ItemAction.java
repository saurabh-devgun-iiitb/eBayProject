...
public class ItemAction extends ActionSupport implements ModelDriven<Item>{
	...
	public String delete() {
		...
		String result = itemDAO.deleteItem(item);
		if(result.equals("true"))
			return SUCCESS;
		else
			if(result.equals("wrongItemDelete"))
				return "WID";
		return ERROR;
	}
	...
	public ArrayList<Item> searchItems(String searchText,String selectedCategory) {
		 System.out.println("TEst in ItemAction1");
		ItemDAO itemDAO = new ItemDAOImplementation();
		if(itemDAO.searchItems(items, searchText,selectedCategory))
			return items;
		return null;
	}
